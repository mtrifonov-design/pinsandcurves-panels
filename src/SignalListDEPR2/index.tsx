...
type ControllerRef = {
    current: TimelineController.TimelineController | undefined;
}
...
type Act =
    ...
    | { type: "FILE_DATA"; id: string; data: unknown; controller: ControllerRef  }
    | { type: "FILE_DELETED"; id: string }
    | { type: "REQUEST_CLOSE_FILE" }
    | { type: "TERMINATE"; blockerId: string }
    | { type: "UNSUB_CONFIRMED"; id: string }
    | { type: "BLOCKER_SENT" }
    | { type: "SUB_UPDATE"; id: string; controller: ControllerRef, update: unknown };   // ★ NEW
    ...
    switch (act.type) {
        ...
        case "FILE_DATA":
            return {
                ...
                file: { ...state.file!, data: act.controller.current },
            };
        ...
        case "SUB_UPDATE":                                  
            ...
            if (state.file?.id === act.id) {
                return { ...state, file: { ...state.file, data: act.controller.current } };
            }
            return state;
...
export default function CodeEditor() {
    ...
    useUnit(unit => {
        ...
        if (payload.getAssetResponse) {
            ...
            if (subscription_id === ctx.index.id) {
                ...
                controller.current = Controller.fromSerializedWorm(asset_data as any);
                controller.current.onPushUpdate(() => {
                    const update = controller.current?.transferOutgoingEvent();
                    if (!update) return;
                    globalThis.CK_ADAPTER.pushWorkload({
                        assetServer: [{
                            ...
                            payload: {
                                updateAsset: {
                                    asset_id: ctx.file?.assetId,
                                    subscription_id: ctx.file?.id,
                                    update,
                                }
                            },
                        }],
                ...
                dispatch({ type: "FILE_DATA", id: subscription_id, data: asset_data, controller });
            }
            return;
        }
    ...
    if (ctx.phase === Phase.OPEN_EDITOR && ctx.file) {
        return <SignalListContent file={ctx.file} controller={controller} index={ctx.index} handleCloseFile={handleCloseFile} />;
    }
