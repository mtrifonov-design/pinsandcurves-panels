...
type Act =
  ...
  | { type: "INDEX_DATA"; id: string; data: unknown; metadata: unknown }
  | { type: "OPEN_FILE_REQUEST"; assetId: string; subName: string }
  | { type: "FILE_CONFIRMED"; id: string }
  | { type: "FILE_DATA"; id: string; data: unknown; metadata: unknown }
...
function reducer(state: Ctx, act: Act): Ctx {
  switch (act.type) {
    ...
    case "INDEX_DATA": {
      const next = {
        ...state,
        index: { ...state.index, data: act.data, metadata: act.metadata },
      };
      if (
    ...
    case "OPEN_FILE_REQUEST":
      return {
        ...state,
        file: { name: act.subName },
      };

    case "FILE_CONFIRMED":
      return { ...state, file: { ...state.file!, id: act.id } };

    case "FILE_DATA":
      return {
        ...state,
        phase: Phase.OPEN_EDITOR,
        file: { ...state.file!, data: act.data, metadata: act.metadata },
      };
    ...
    case "UNSUB_CONFIRMED": {
      const next = { ...state };
      if (state.index.id === act.id) next.index = { ...state.index, unsubConfirmed: true };
      if (state.file?.id === act.id) next.file = { ...state.file, unsubConfirmed: true };
      return next;
    }
...
export default function CodeEditor() {
  ...
  useUnit(unit => {
    ...
    if (payload.receiveUpdate) {
        ...
        console.log("receiveUpdate", subscription_id, update);
    }
    ...
    if (payload.TERMINATE && ctx.phase !== Phase.READY_TERMINATE) {
      ...
      if (ctx.file?.id)  sendUnsubscribe("file",  ctx.file.id); // assetId "file" is placeholder
      return;
    }
    ...
    if (payload.subscriptionConfirmation) {
      ...
      if (subscription_name === ctx.index.name) {
        ...
        dispatch({ type: "FILE_CONFIRMED", id: subscription_id });
      }
      return;
    }
    ...
    if (payload.getAssetResponse) {
      const { subscription_id, asset_data, asset_metadata } = payload.getAssetResponse;
    ...
    //   console.log("-----");
      ...
      if (subscription_id === ctx.index.id) {
        dispatch({ type: "INDEX_DATA", id: subscription_id, data: asset_data, metadata: asset_metadata });
      } else if (subscription_id === ctx.file?.id) {
        dispatch({ type: "FILE_DATA", id: subscription_id, data: asset_data, metadata: asset_metadata });
      }
      return;
    }
  ...
  if (ctx.phase === Phase.LOBBY) {
    ...
    return <div>Lobby - pick a file
        ...
        <pre>{JSON.stringify(ctx.index.data,null,2)}</pre>

    </div>;
  }

  if (ctx.phase === Phase.OPEN_EDITOR && ctx.file) {
    return <div>{/* real editor */}Editing …</div>;
  }
