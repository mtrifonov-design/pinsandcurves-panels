...
type Act =
    ...
    | { type: "FILE_DATA"; id: string; data: unknown; controller: unknown  }
    | { type: "FILE_DELETED"; id: string }
    | { type: "REQUEST_CLOSE_FILE" }
    | { type: "TERMINATE"; blockerId: string }
    | { type: "UNSUB_CONFIRMED"; id: string }
    | { type: "BLOCKER_SENT" }
    | { type: "SUB_UPDATE"; id: string; controller: unknown, update: unknown };   // ★ NEW
...
function reducer(state: Ctx, act: Act): Ctx {
    switch (act.type) {
        ...
        case "FILE_DATA":
            ...
            act.controller.current = Controller.Host((e) => {
                globalThis.CK_ADAPTER.pushWorkload({
                    assetServer: [{
                        ...
                        payload: {
                            updateAsset: {
                                asset_id: state.file?.assetId,
                                subscription_id: state.file?.id,
                                update: e,
                            }
                        },
                    }],
            ...
            return {
                ...
                file: { ...state.file!, data: act.controller.current.getProject() },
            };
        ...
        case "SUB_UPDATE":                                  
            ...
            if (state.file?.id === act.id) {
                act.controller.current.receive(act.update);
                return { ...state, file: { ...state.file, data: act.controller.current.getProject() } };
            }
            return state;
