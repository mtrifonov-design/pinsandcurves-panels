...
export default async function parseScript(raw: string, baseURL? : string): Promise<ParsedEntry[]> {
  ...
  const entries = await parseDeclarations(step3, sectionMap, glslMap, baseURL);
...
async function parseDeclarations(
  ...
  for (const part of parts) {
    ...
    let resolved = await deepReplaceGlslPlaceholders(data, glslMap, `${name}:${kind}`, base);
...
async function deepReplaceGlslPlaceholders(value: any, glslMap: GLSLMap, ctx: string, base?: string): Promise<any> {
  if (typeof value === "string") {
    ...
    return await resolveGLSL(code, base);
  }
  if (Array.isArray(value)) {
    return Promise.all(value.map((v) => deepReplaceGlslPlaceholders(v, glslMap, ctx, base)));
  }
  if (value && typeof value === "object") {
    const out: Record<string, any> = {};
    for (const [k, v] of Object.entries(value)) {
      out[k] = await deepReplaceGlslPlaceholders(v, glslMap, ctx, base);
    }
    return out;
  }
  return value;
