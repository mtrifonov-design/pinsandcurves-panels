...
import { getResourceChanges, processResources } from "./compile";

export default class Graphics {
    resources: Map<string, ResourceClass> = new Map();
    ...
    compile(newResourceBlueprint: any[]) {
        const { toDelete, toAdd } = getResourceChanges(newResourceBlueprint, this.resourceBlueprint);
        // iterate over toDelete
        toDelete.forEach(resourceBlueprint => {
            const resource = this.resources.get(resourceBlueprint.id);
            if (!resource) throw new Error("Something went wrong.");
            resource.dispose();
