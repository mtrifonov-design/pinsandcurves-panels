...
const colorTextureStream = new CachedStream();
function colorTextureStreamReducer(state: ControlsData) {
    ...
    colorTextureStream.updateStream([{
        resource: "cyberspag_colorTexture",
        type: "setTextureData",
        payload: [colorBuffer]
    }]);
    return colorTextureStream.getStream();
}

export default colorTextureStreamReducer;
